...
const FREEMODE_CLOTHING_DLC_RE = /^mp_[mf]_freemode_01_mp_[mf]_[^/]+$/i;
const FREEMODE_PED_RE = /^mp_[mf]_freemode_01$/i;
...
const META_DATA_FILE_TYPES: Array<{ pattern: RegExp; type: string }> = [
  ...
  // 服 addon は Freemode DLC 名の .ymt/.meta を SHOP_PED_APPAREL_META_FILE として
  // 読ませる必要がある。これが無いと ydd/ytd は stream されても衣装スロットに出ない。
  { pattern: /(^|\/)shop_ped_apparel[^/]*\.(meta|ymt)$/i, type: "SHOP_PED_APPAREL_META_FILE" },
  { pattern: /(^|\/)mp_[mf]_freemode_01_mp_[mf]_[^/]*\.(meta|ymt)$/i, type: "SHOP_PED_APPAREL_META_FILE" },
  { pattern: /(^|\/)shop_ped_outfits[^/]*\.(meta|ymt)$/i, type: "SHOP_PED_OUTFIT_META_FILE" },
  { pattern: /(^|\/)alternate(?:_?variation|variation)s?[^/]*\.(meta|ymt)$/i, type: "ALTERNATE_VARIATIONS_FILE" },
  ...
  for (const [resourcePath, list] of grouped) {
    ...
    if (uniqueHashes.size === 1) {
      ...
      issues.push({
        severity: "warning",
        path: resourcePath,
        message: `中身が異なる${list.length}個のファイルが同じ出力パス(${resourcePath})になります。採用するファイルを選ぶか、フォルダ単位で別リソースに分割してください。`,
      });
    }
  }
...
export async function analyzeSourceFiles(
  ...
  const rootNameGuess =
    detectClothingResourceName(resolved) ??
    detectVehicleModelName(resolved) ??
    sanitizeResourceName(sourceFileName.replace(/\.(zip|rpf)$/i, ""));
...
function isStructuralSegment(segment: string): boolean {
  ...
  if (isClothingStreamContainerSegment(lower)) return false;
...
export function buildFxManifest(files: ResolvedFile[]): string {
  ...
  const lines = [
    ...
    "game 'gta5'",
    "",
    "author 'Generated by FiveM Resource Fixer'",
...
function detectClothingResourceName(files: ResolvedFile[]): string | null {
  const counts = new Map<string, number>();
  for (const file of files) {
    for (const path of [file.sourcePath, file.resourcePath]) {
      const segments = path.split("/");
      for (const segment of segments) addClothingDlcName(segment, counts);
      addClothingDlcName(getBaseName(path).replace(/\.(meta|ymt)$/i, ""), counts);
    }
  }
  if (counts.size === 0) return null;
...
function addClothingDlcName(segment: string, counts: Map<string, number>): void {
  const normalized = stripRpfExtension(segment);
  const match = normalized.match(/^mp_[mf]_freemode_01_mp_[mf]_(.+)$/i);
  if (!match) return;
  const name = sanitizeResourceName(match[1] ?? "");
  if (!name) return;
...
// FiveM リソースは stream/ と data/ を取る。通常の武器/車両 stream は basename
// だけで足りるが、服 addon は mp_*_freemode_01_mp_*_* フォルダを維持しないと
// 男性/女性や DLC パック単位の同名 ydd/ytd が衝突するため、該当パスだけ保持する。
function getFiveMResourcePath(path: string): string {
  const ext = getExtension(path);
  const base = getBaseName(path);
  if (STREAM_EXTENSIONS.has(ext)) {
    const clothingPath = getClothingStreamRelativePath(path);
    return `stream/${clothingPath ?? base}`;
  }
  if (DATA_EXTENSIONS.has(ext)) return `data/${base}`;
  return `stream/${base}`;
}

function getClothingStreamRelativePath(path: string): string | null {
  const segments = path.split("/");
  for (let i = 0; i < segments.length - 1; i++) {
    const segment = segments[i]!;
    if (!isClothingStreamContainerSegment(segment)) continue;
    return [stripRpfExtension(segment), ...segments.slice(i + 1)].join("/");
  }
  return null;
}

function isClothingStreamContainerSegment(segment: string): boolean {
  const normalized = stripRpfExtension(segment);
  return FREEMODE_CLOTHING_DLC_RE.test(normalized) || FREEMODE_PED_RE.test(normalized);
}

function stripRpfExtension(segment: string): string {
  return segment.replace(/\.rpf$/i, "");
}
